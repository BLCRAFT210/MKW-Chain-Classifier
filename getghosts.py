...
with open('leaderboard.json', 'r+') as f:
    ...
    for n, entry in enumerate(data['ghosts'][:NGHOSTS]):
        if entry['hash'] in leaderboard:
            print('Skipping run '+str(n+1)+' with hash '+entry['hash']+': already downloaded')

        elif entry['vehicleId'] != 32 or entry['driverId'] != 22:
            print('Skipping run '+str(n+1)+' with hash '+entry['hash']+': not set with Funky Kong on the Spear')

        else:
            print('Downloading run '+str(n+1)+' with hash '+entry['hash']+'...')
